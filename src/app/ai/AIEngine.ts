...
export class AIEngine {
  ...
  private easyStrategy(
    ...
    // 随机出牌
    const playableCards = hand.filter((card) => card.cost <= mana && board.length < 7);
  ...
  private normalStrategy(
    ...
    const playableCards = hand
      .filter((card) => card.cost <= remainingMana && board.length < 7)
      .sort((a, b) => (b.cost || 0) - (a.cost || 0));

    for (const card of playableCards) {
      if (card.cost <= remainingMana && board.length < 7) {
        commands.push({
          type: "PlayCard",
          playerId: "ai",
          cardId: card.id,
