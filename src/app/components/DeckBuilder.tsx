...
export function DeckBuilder({ onClose, onSaveDeck, initialDeck }: DeckBuilderProps) {
  ...
  const deckStats = useMemo(() => {
    ...
    selectedCards.forEach((cardId) => {
      const template = Object.values(CARD_DATABASE).find((c) => cardId.startsWith(c.id));
      if (template) {
    ...
    const avgCost =
      selectedCards.reduce((sum, cardId) => {
        const template = Object.values(CARD_DATABASE).find((c) => cardId.startsWith(c.id));
  ...
  const addCard = (cardId: string) => {
    ...
    if (selectedCards.length < 30 && cardCount < 2) {
      setSelectedCards([...selectedCards, `${cardId}_${Date.now()}`]);
    }
  };
  ...
  const handleSave = () => {
    if (selectedCards.length === 30) {
      onSaveDeck({ name: deckName, cards: selectedCards });
      onClose();
    }
  };
          ...
          <div className="flex-1 p-6 overflow-auto">
            ...
            <div className="grid grid-cols-4 gap-4">
              {filteredCards.map((card) => {
                const cardCount = selectedCards.filter((id) => id.startsWith(card.id)).length;
                return (
                  <motion.div
                    key={card.id}
                    whileHover={{ scale: 1.05 }}
          ...
          <div className="w-96 bg-gray-900/50 border-l border-gray-700 p-6 overflow-auto">
            ...
            <div className="space-y-2">
              {selectedCards.map((instanceId) => {
                const templateId = instanceId.split("_")[0];
                const template = Object.values(CARD_DATABASE).find((c) => c.id === templateId);
